...
**Build scope:** Tier A + Tier B = **AC-1 – AC-169**. AC-170 – AC-224 are modeled where cheap, not built (§10).
...
Writer: an `afterWrite` hook on every mirrored table. Reader: queue consumer batching **10 records per PATCH** with `performUpsert.fieldsToMergeOn: ["marquee_id"]`, rate-limited to 4 req/s.
...
Writer: drain, webhook receive, daily keepalive cron (trap 7). Reader: **Settings → Airtable** page — connected base link, both row counts, last sync, outbox depth, Sync now (seams §1.5).
...
Every row carries `last_write_source` ∈ `marquee\|airtable` to break echo loops. Inbound is *pull-triggered-by-ping*: the webhook POST carries no data; the Worker calls list-payloads with the stored cursor (seams §1.2).
...
- `POST /api/v1/admin/reset-demo` → restores the seeded state, idempotent, safe mid-judging (`EVALUATION.md` gate 13). Also a button in the product.
...
*Covers AC-25, AC-26, AC-29, AC-30 – AC-42, AC-155 – AC-157.* Public shell (event wordmark, `Resume draft`). Header block: "Call for speakers · closes Sep 12", headline, welcome copy, progress dots. Fields in builder order, with character counters where a max exists, the speaker-limit sentence **before** the first add-person control (AC-29), Turnstile, and a footer row: left `Draft saved locally · just now` (AC-41), right `Submit abstract`.
...
*Covers AC-43 – AC-52, AC-146 – AC-148, AC-152 – AC-154.* Public-ish shell with `Sign out`. Head: "Welcome back" + name + tasks-done progress. **Status hero is the most prominent element on the screen (AC-43):** `Accepted · Wave 2` eyebrow, session title, `Format · Day · Time · Room`, `[Add to calendar]`. Pre-decision it reads a concrete next-wave date, never blank and never the bare word "pending" (AC-44).
Below: **Your tasks** (checkbox, title, `kind · due`, state chip; ordered by due date; overdue rows carry a textual `Overdue` marker) and a right stack: **Speaker profile** (avatar, name, title·company, bio, `Edit` → inline edit of bio/headshot/title/company/social links, with a crop preview before save) and **Speaker handbook** (two handbook pages) **[beyond v1.1 — flag F-3]**.
...
| Settings → Airtable (base link, both row counts, last sync, outbox depth, Sync now, live log) | `/settings/airtable` | **no AC — flag F-1** |
...
| **G4** | **Airtable is never read on a request path** | Static check: the Airtable client is importable only from `src/jobs/mirror/*`; a lint rule fails any import elsewhere. Runtime: every mirror call is tagged and `check:speed` asserts zero mirror calls during page renders | **A-4 · mirror isolation** |
...
| **G8** | **Public writes are bot-defended and upload-safe** | Turnstile verified server-side before any public write or presign; extension + MIME allowlist; **magic-byte sniff on completion**; per-IP and per-submission upload caps in KV; uploads served from a separate origin with `Content-Disposition: attachment` and `X-Content-Type-Options: nosniff` | **A-7 · public write surface** (no AC — flag F-6) |
...
| **G12** | **Demo resettable between judges** | `npm run reset:demo` ≤20 s, idempotent, safe mid-judging; second judge inherits nothing (gate 13) | **A-11 · reset drill** |
...
Deviations and gaps, raised rather than silently forked (living-artifacts norm). Each needs a ruling at client review.

| # | Flag | Why it matters | Recommendation |
|---|---|---|---|
| **F-1** | **The Airtable two-way mirror has no acceptance criterion.** It is `EVALUATION.md` gate 9, the competition's *larger* stack bonus, and a named architecture bet — and no AC in `USER_STORIES.md` covers it. | An uncovered gate item is exactly what gets cut under time pressure and then fails the gate. | Mint **AC-225 – AC-229** at review: outbound upsert within 60 s of a local change · inbound allowlisted field applies within one webhook cycle · echo suppression (a mirrored write does not bounce back) · Settings → Airtable shows base link, both row counts, last sync, outbox depth · webhook keepalive cron survives 7 days. |
| **F-2** | **Seed scale collides with AC-3.** AC-3 requires ≥150 accepted speakers; the recommended faithful seed (Option A) yields 75. | Tier A admits no waivers, so this is a hard fail as written. | Ship **A′** — the 60 real Feb-2025 sessions plus the real, non-overlapping CODE-2025 roster, reaching ~150 accepted speakers with zero fabricated accepted people. Client confirms at review. |
| **F-3** | **Speaker Handbook page** (brief item 8, "cheap yes" in `PRODUCT-DEFINITION.md` §3, present in the v1.1 portal) has no story and no AC. | It is visible in the binding design, so shipping without it is a prototype-fidelity defect; building it uncovered is unbudgeted. | Two handbook pages, static markdown per event, below the Tier B cut line. Mint an AC or record it as a deliberate non-goal. |
| **F-4** | The v1.1 prototype renders several loop-critical affordances as toasts (public session/speaker permalinks, admin manual entry, plan/scorecard/committee editing, add break, export). | §5.13 specifies them, but the prototype is the binding contract and it does not show them. | Orchestrator re-verifies §5.13 against the final v1.1 at client review; anything the client wants different is a prototype amendment, not a build decision. |
| **F-5** | `reset:demo` is `EVALUATION.md` gate 13 and a product button, with no AC. | Same shape as F-1. | Fold into the F-1 amendment batch. |
| **F-6** | **Turnstile, upload magic-byte sniffing, and per-IP caps** (guardrail G8) have no AC. | R19 puts an open write endpoint on the internet for four days with a public repo pointing at it. | Build regardless (it is ~2 h); mint an AC or record as a guardrail-only obligation. |
| **F-7** | **AC-89 (embed reflects a change within 60 s) vs KV caching.** | A 60 s KV TTL leaves no headroom against a 60 s budget. | Spec'd at a **30 s TTL with explicit purge on publish**. Recorded here so the number is a decision, not an accident. |
| **F-8** | **Live-ness mechanism for AC-14 / AC-94** is a 5 s SWR poll, not a push channel. | "Real-time" in the brief; polling is the honest, low-risk read of it inside the window. | Accept; revisit only if the Sunday video says otherwise. |

---

*Draft, 2026-08-08. Amendments follow `USER_STORIES.md` rules: new criteria append from AC-225; deletions are struck, never recycled. Next inputs — orchestrator review with the client, the Sunday clarification video (requirements freeze), Discord rulings on Q1/Q2, and final `pipeline-v1.1` sign-off as the binding visual contract.*
