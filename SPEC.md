...
Below: **Your tasks** (title, `kind · due`, state chip, `Finish now`/`View`; ordered by due date; overdue rows carry a textual `Overdue` marker). **[Beyond the v1.4 prototype — acknowledged divergence, operator ruling 2026-08-15: build per spec; fidelity audit must not flag it.]** Both prototypes render this button as `Complete`/`Update`. `Complete` is struck as a button label because the state chip on a finished row already reads `Complete`: one word doing both jobs on one screen, so a filled primary button wearing it reads as a badge announcing the task is done rather than the instruction to go do it. `Complete` survives as the **status** only. `Update` is struck with it — a done row's action opens the same payload read-only, so `View` is what pressing it does. This binds the sponsor portal's identical row (§ MRQ-214) too: the two seats share one `TaskRow`, and a build that reproduces `prototypes/sponsor-portal/index.html` literally would reintroduce the defect. **Cancelled tasks (AC-265)** are out of this list and out of the `N of M tasks done` progress figure, rendered instead below a dashed divider under `Cancelled · N` with the reason stated **once** — a speaker whose talk was withdrawn is told why their homework vanished, not left to notice. A cancelled row carries no action button. **Only open tasks are ever cancelled**, so work the speaker already finished stays in the active list, still reading as done: the progress figure falls to `2 of 2` rather than erasing what they did. Opening a task renders its real kind: agreement acknowledgment, required form fields, or direct file upload; only a valid kind-specific payload completes it. The right stack is **Speaker profile** (avatar, name, title·company, bio, `Edit` → bio/headshot/title/company/social links, with crop preview before save) and **Speaker handbook** — handbook pages authored as static markdown per event, rendering inside the portal (**AC-233**, US-39). **AC-233 sits below the Tier B cut line**: it may be cut, and if it is, the gate report names it with its AC ID and reason (gate 19). Silently missing is a failure.
...
**Roster count ruling (2026-08-15, post-Amendment-7 clarification):** For event-scoped roster and onboarding list responses, `total` is the number of rows matching the active filters across all pages, and `total_pages` derives from that filtered total. When a response exposes status facets, `counts.all` is the unfiltered roster size. This intentionally supersedes the pre-Amendment-7 behavior where `total` stayed at roster size after a search or filter; a narrowed view may therefore truthfully report “shown of `counts.all`.”
...
**Acknowledged divergences from the prototype.** Six, each for a stated reason; everything else reproduces one-to-one.

1. **Tasks and handbook chapters expand in place** rather than opening modals. The shipped task-row machinery is what the build reproduces, and a second completion UI inside one product is worse than a matched animation.
2. **The switcher is real.** The prototype's dashed switcher is prototype chrome; a sponsorship switcher renders in its place, and only for a contact who genuinely holds more than one deal.
3. **A file deliverable states vector PDF**, not "SVG or EPS". `task_upload` narrows `DOCUMENT_RULES`, which sniffs pdf/pptx/key and nothing else, so an accept list naming `.svg` presigns nothing and the sponsor meets "that file type is not accepted" after being told to send one. Widening the sniffer is a security-surface change and wants its own ticket.
4. **The progress meter is the speaker portal's numeral gauge**, not the prototype's fill bar — the same instrument both portals read, in tabular mono so it cannot reflow.
5. **No completion toast.** The row turns done, its attribution line fills in, and the meter moves; the shipped speaker portal confirms completions the same way, and a toast on top of three visible changes is a fourth.
6. **No "Add to calendar" on a Session card.** The prototype offers one. A calendar invite is per-participation and belongs to the person presenting; a sponsor contact is not, and the only ICS this product serves without one is the *public* feed, which an unpublished sponsor Session must not appear in. Offering a sponsor-facing invite is real machinery and its own ticket, not a button.

Two smaller notes, ratified rather than fixed: sponsor Sessions carry the conference's own real format names (§6's four, verbatim from the CFP) rather than the prototype's invented "Sponsored Talk"; and the type scale is the shipped portal's, not the prototype's px, so the two portals read as one product.

---

## Amendment 24 — the Airtable mirror is reinstated, built hermetic *(operator ruling, 2026-08-15)*

**Amendment 17 is superseded.** MRQ-26, MRQ-27, MRQ-46 and MRQ-54 were cancelled on 2026-08-11 for one reason and one reason only — no Airtable base was in hand and the deploy was unshipped, so five of eleven remaining tickets sat blocked on a track that had not started. The deadline is behind us, the deploy is live, and the blocking condition no longer applies. **§3.9 stands exactly as written and is now in build scope**; AC-225 – AC-229 are live criteria again. Nothing in §3.9 is re-designed by this amendment.

**The credential is decoupled from the build (the ruling that makes this different from August).** The mirror is built against an **injected Airtable transport** — an interface, with the real `fetch`-backed implementation on one side and a fake on the other. `npm test` therefore covers every criterion, including the round-trip and the echo-suppression loop, without a token, a base, or a network. A missing credential is a **precondition, not a blocker**: it decides when `check:mirror` can run against a real base, not whether the code exists. This is the specific mistake of the first attempt — a track that could not start because a human had not finished an errand — and it does not get made twice.

**Absent configuration disables the mirror cleanly.** With no `AIRTABLE_API_KEY` or no configured base, the change feed does not accumulate, the consumer no-ops, the webhook route 404s, and **Settings → Airtable says "not connected" and offers the connect path**. It never errors, never retries into a queue backlog, and never appears half-built. A product with the mirror switched off must be indistinguishable from today's product, which is what lets this land before the base exists.

**What the honest positioning becomes.** Amendment 4's trade is unchanged and still true — D1 is the source of truth, Airtable is never on a read path, and the source-of-truth bonus was deliberately forgone. What changes is only that the extension point named in the README becomes a shipped surface. The README's extension-points table is updated by the inbound ticket, in the same PR that makes the sentence false.
...
**Tickets.** MRQ-217 (outbound, merged 2026-08-15) and MRQ-223 (the finisher: connect, inbound, keepalive, Settings → Airtable, and the agent surface; supersedes the rolled-up MRQ-218). The mirror-isolation audit that MRQ-46 would have performed is folded into MRQ-217 as the import-boundary lint it was going to check for, because a guardrail that ships with the code it guards cannot be skipped for capacity. MRQ-54's spike is not reinstated as a separate item: with an injected transport, the inbound loop is exercised by the suite, and the one thing a spike would have told us that the fake cannot — whether Airtable's real webhook payload matches its documentation — is answered the first time a base is connected.
