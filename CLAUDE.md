...
- **The suite budget is 45s and the gate budget is 120s** (`scripts/checks/run-test.mjs`, `pr-gate.mjs`) — **objectives that print loudly and pass, not gates.** Slowness fails nothing in those two: over budget reports `pass-over-budget`, and `process.exitCode` comes from the test outcome. Only `HARD_LIMIT_MS` (600s, a hang detector) turns a slow run red, and it reports `timeout` rather than `fail`. So **read the status field rather than judging the machine**:
  ...
  **The CI ceiling has almost no headroom left, so read a CI `timeout` as expected rather than as a signal — and the figure is now measured rather than folklore.** Six `elapsedMs` samples read out of fast-gate logs on 2026-08-18: `main` at **555,829 / 587,733 / 589,568** and a branch at **585,143 / 600,041 (timeout) / 564,490**. So the suite runs **~556-590s against the 600,000ms kill — a 10-44s margin — and it will keep tripping.** Two consequences worth holding. First, **the same sha varies by ~35s**: `2af302b4` timed out at `600,041ms` and then passed at `564,490ms` on a re-run of identical code, which is why re-running eventually goes green on anything and why a green re-run is not evidence your branch is fast. Second, **`main` itself is inside the tripping band** (its own recent run left 10.4s), so a CI `timeout` is nearly always about the runner and the base, not your diff. Budget for one re-run of the same sha. The reverse error is the expensive one: this is exactly the reflex that ends in a real `fail` being waved through as "the known flaky ceiling", so keep `fail` and `timeout` strictly apart — `fail` is load-invariant and blocks, whatever the box was doing.

  **What the ceiling is NOT caused by, established by experiment (MRQ-280, PR #332):** it is not one slow file and not per-file overhead. Removing **six** Worker-pool files (121 to 115) moved CI by **4.4s against a 34s spread** — nothing — while preserving all 1,781 tests. The often-quoted **~40s per workers-pool file is noise** and is disconfirmed: it came from two single runs, one of them censored at the 600s ceiling. The real shape, measured: a Worker-pool file costs **~4.3s** against **~0.25s** for a node-pool file (**17x**), the vitest phase is **~563s of a 585s total** (`node --test` is only ~37s), and **`import` alone is ~288s of ~552s** — over half the suite is module transform, not test execution. Import cost tracks the **unique module graph**, not the file count, because Vite shares transform results across files in a process. So the remaining lever is entirely **inside vitest** (file-level parallelism and import cost) and is **not** per-file trimming. Note `poolOptions.workers.isolatedStorage` is **not** a supported setting on Vitest 4.1.10 with Cloudflare pool 0.20.3. **MRQ-280 stays open** and owns the duration; do not read PR #332's merge as headroom restored.
...
- **D1 caps a compound SELECT at FIVE terms, not SQLite's documented 500.** Measured 2026-08-18 by binary search on both engines: workerd/D1 (the vitest workers pool — the same engine as `npm test`, the local gate and hosted `fast-gate`) and **production D1 over the API**. Both accept 5 arms and fail the 6th with `too many terms in compound SELECT: SQLITE_ERROR` (`[code: 7500]` remotely). Local and prod agree, so the gate is a faithful proxy — but any compound chain over 5 that reaches prod is a 500 waiting.
  - **The limit is PER CHAIN, and nesting resets it.** Top level, a subquery, a CTE body and an `IN (...)` each get their own 5. Flat 6 fails; 5 subqueries × 5 arms (25 total) passes; 6 subqueries fails because the *outer* chain is 6. So wrapping in a subquery is a real escape hatch and one level buys 25.
  - It applies to `UNION`, `UNION ALL`, and literal or table selects alike — there is no variant that escapes it.
  - **Assume no query may grow its arm count with the schema.** A chain with one arm per referencing table is a time bomb: it passes at 5 tables and breaks the day someone adds the sixth, in production, on a central operation. MRQ-286 hit exactly this — one new arm took `person-merge.ts`'s 5-arm chains to 6 and killed people-merge with a 500 *after* a preview promising success. Prefer bounded per-table `EXISTS` predicates, or independent SELECTs combined in JS, over a generated compound chain.
  - Adding an arm to an existing chain? Count the terms first. `git grep -n "UNION"` and check nothing is already at 5.
