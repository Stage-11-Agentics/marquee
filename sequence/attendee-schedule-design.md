...
### Round 4 — build rulings (Atin, 2026-08-14 · v0.3 loved, design signed off)

Atin drove v0.3 in Chrome and signed off, round-3 additions included. **Prototype v0.3 is the binding visual contract for the build.** Five rulings close the open questions:

- **Claim is request → verify.** Typing an email and pressing Send only sends the mail (Turnstile-gated, rate-limited). The CRM write (person upsert + attendance row), the identity-line flip, and the speaker pins all happen when the emailed sync link is first opened — the mail already carries that link, so the verification click costs nothing new. Until then the claim row shows a pending state ("Check your email — link sent to …") and the page stays anonymous. This closes the impersonation hole the speaker cross-over would otherwise open: typing a speaker's email must not conjure their pins or attribute picks to them. The v0.3 prototype shows the post-verify state; the pending state is a small, acknowledged build divergence.
- **Retention: persist indefinitely.** Schedule codes and webcal feeds outlive the event — a feed that goes quiet beats a calendar that breaks. No TTL machinery.
- **Demand board home: a panel of the Agenda module** in the admin shell — demand is agenda-shaped data (sessions × rooms) — with the public-counts setting beside it. The import-prompt block lives on the People surface as ruled above. The prototype's public-footer door remains a demo convenience only.
- **Speaker pins are derived, never stored.** The person-match derives the pins at render; they never enter the stored `sessionIds`. Unstar cannot touch them; unlink removes them with the identity. They ride the ICS/webcal set and the agent briefing; they do **not** appear in the shared read-only link — a friend importing your picks must not inherit "you're speaking."
- **Build shape: one solid ticket, one Opus delegator** (round-1 precedent, MRQ-132). Claim mail remains launch-gated on the Resend tier per Constraints — build ≠ enable.
