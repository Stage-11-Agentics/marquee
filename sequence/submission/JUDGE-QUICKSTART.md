...
the program pipeline (submitted → in review → waved → ready to place →
onboarding → scheduled → published), the wave planner, and the chase numbers —
including how many decided submissions have not yet been notified.
...
Open one, or press **+ New form**: seven build steps (type & basics, welcome,
form fields, participants, rules & routing, messages, publish). Nine field
types, per-field required/validation, conditional visibility, and track
routing. Format and track options are bound to Settings, not hand-typed.
...
Program fit 40% · Audience value 35% · Clarity 25%, plus a recommendation and
comments; round 2: final score + committee notes), anonymous-review toggles per
round — round 1 anonymous, round 2 identity visible — reviewer pools, 3 reviews
...
Sign out, **Enter as reviewer**. You are the Triage agent — the AI evaluator
seat station 6 mints — and your queue holds only submissions in your authorized
tracks — the scoping is enforced at the API, not hidden in the UI. Review
anonymously with **Approve / Maybe / Deny** (keyboard:
...
As organizer, open
[/agenda-builder](https://marquee.stage11.dev/agenda-builder). Place accepted
sessions from the unscheduled tray into a day, time, and room — drag them, or
select one and click the slot — format defaults set the duration; placement
persists as it happens, no save button. Five views:
...
- **The API is the product.** `curl https://marquee.stage11.dev/api/openapi.json`
  — 211 operations, OpenAPI 3.1, and the `ETag` header is the SHA-256 of the
