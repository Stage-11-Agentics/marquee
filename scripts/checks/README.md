...
The private forge has no CI runner. This command is the merge evidence: Worker/client/test type checks, the production build, design-contract verification, `npm test`, and merged-scope AC tracing. The public workflow mirrors those fast checks but is not evidence for a private review.
