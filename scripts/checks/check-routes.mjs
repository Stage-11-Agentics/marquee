...
const COMPOSITION_ROOT = resolve(REPOSITORY_ROOT, "src/index.ts");
...
/**
 * The machine doors the composition root registers itself — `/health`, `/mcp`,
 * `/SKILL.md` and friends. They are neither SPA routes nor `*.route.tsx` pages,
 * so the three existing sources cannot see them, and a hand-kept list of them
 * would rot exactly the way the page list did. `MCP_PATH`-style constants are
 * resolved against their declarations so a renamed endpoint moves here with it.
 */
async function machineEndpoints() {
  const source = await readFile(COMPOSITION_ROOT, "utf8");
  const constants = new Map(
    [...source.matchAll(/const (\w+) = "(\/[^"]*)"/g)].map((match) => [match[1], match[2]]),
  );
  // A path declared beside its handler and imported here (`MCP_PATH`) is the
  // better place for it to live, so the scan follows the import rather than
  // demanding the literal be repeated in this file to stay visible.
  for (const importMatch of source.matchAll(/import \{([^}]*)\} from "(\.[^"]*)"/g)) {
    const names = importMatch[1].split(",").map((name) => name.trim()).filter((name) => /^[A-Z][A-Z0-9_]*$/.test(name));
    if (names.length === 0) continue;
    const moduleSource = await readFile(resolve(REPOSITORY_ROOT, "src", `${importMatch[2].replace(/^\.\//, "")}.ts`), "utf8").catch(() => null);
    if (moduleSource === null) continue;
    for (const declaration of moduleSource.matchAll(/export const (\w+) = "(\/[^"]*)"/g)) {
      if (names.includes(declaration[1]) && !constants.has(declaration[1])) {
        constants.set(declaration[1], declaration[2]);
      }
    }
  }
  const found = new Map();
  for (const match of source.matchAll(/\bapp\.(get|post|put|patch|delete|all)\(\s*(?:"(\/[^"]*)"|(\w+))/g)) {
    const path = match[2] ?? constants.get(match[3]);
    if (path === undefined || path === "*" || path.startsWith("/api/")) continue;
    found.set(path, [...new Set([...(found.get(path) ?? []), match[1].toUpperCase()])]);
  }
  return [...found.entries()]
    .map(([path, methods]) => ({ path, methods: methods.sort().join(", ") }))
...
function renderRoutesBlock(spa, pages, machine) {
  ...
  lines.push(`  Machine endpoints: ${machine.map((endpoint) => endpoint.path).join(" ")}`);
  return lines.join("\n");
}

function renderDocument({ spa, pages, predicate, machine }) {
  ...
  const machineRows = machine.map((endpoint) => `| \`${endpoint.path}\` | ${endpoint.methods} |`);
...
<!--
  ...
  src/routes/*.route.tsx, src/index.ts. The PR gate fails when this file drifts
  from them.
-->
...
\`\`\`text
${renderRoutesBlock(spa, pages, machine)}
\`\`\`
`;
}
...
const machine = await machineEndpoints();
...
const document = renderDocument({ spa, pages, predicate, machine });

if (args.write) {
  await writeFile(OUTPUT, document, "utf8");
  emit({ command: "check:routes", status: "pass", wrote: "docs/ROUTES.md", spaRoutes: spa.length, serverPages: pages.length, machineEndpoints: machine.length });
} else {
