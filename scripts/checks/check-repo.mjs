...
const objectList = await run("git", ["-C", repository, "rev-list", "--objects", String(args.ref)]);
const paths = objectList.stdout.split("\n").map((line) => line.slice(line.indexOf(" ") + 1)).filter((line) => line && !/^[0-9a-f]{40,64}$/.test(line));
