 ...
 * R7 makes those measured product behaviors graded. Local and quiet-box runs
 * use the canonical AC budgets; the hosted GitHub runner has an explicit,
 * runner-specific calibration for its browser wall clock, and still fails when
 * that calibrated ceiling is missed. Objective-only measurements remain
 * warning-only through classifySpeedMeasurements. The check:seed wrapper has a
 * separate warning-only boot objective because its wall clock is dominated by
 * starting Wrangler rather than by the seeded behavior it verifies.
 */
...
const runner = args.runner === undefined ? (process.env.MARQUEE_SPEED_RUNNER ?? "default") : String(args.runner);
let result;
if (args.input) {
  ...
  const classified = classifySpeedMeasurements(measurements, { gate: isGateRun(), scope, runner });
  result = {
    command: "check:speed",
    scope,
    runner,
  ...
  result = await runSpeedCheck({ gate: isGateRun(), scope, runner });
}
