...
async function buildFixture(): Promise<void> {
  const now = Date.now();
  const sql = `
    CREATE TABLE IF NOT EXISTS events (id TEXT PRIMARY KEY, name TEXT NOT NULL, slug TEXT NOT NULL, timezone TEXT NOT NULL);
    ...
    CREATE TABLE IF NOT EXISTS submissions (id TEXT PRIMARY KEY, reference_code TEXT, event_id TEXT NOT NULL, kind TEXT NOT NULL, title TEXT NOT NULL, abstract TEXT, status TEXT NOT NULL, format_id TEXT, wave_id TEXT, origin TEXT NOT NULL, submitted_at INTEGER, updated_at INTEGER NOT NULL, search_blob TEXT NOT NULL DEFAULT '');
    CREATE TABLE IF NOT EXISTS people (id TEXT PRIMARY KEY, name TEXT NOT NULL, email TEXT, company TEXT);
    ...
    CREATE TABLE IF NOT EXISTS buildings (id TEXT PRIMARY KEY, event_id TEXT NOT NULL, name TEXT NOT NULL, address TEXT, lat REAL, lng REAL);
    CREATE TABLE IF NOT EXISTS rooms (id TEXT PRIMARY KEY, event_id TEXT NOT NULL, building_id TEXT NOT NULL, name TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS agenda_items (id TEXT PRIMARY KEY, event_id TEXT NOT NULL, submission_id TEXT, kind TEXT NOT NULL, starts_at INTEGER NOT NULL, duration_min INTEGER NOT NULL, room_id TEXT NOT NULL, is_published INTEGER NOT NULL);
    ...
    CREATE TABLE IF NOT EXISTS calendar_invites (id TEXT PRIMARY KEY, submission_id TEXT NOT NULL, person_id TEXT NOT NULL, uid TEXT NOT NULL, sequence INTEGER NOT NULL, last_method TEXT NOT NULL, status TEXT NOT NULL, request_snapshot TEXT, organizer_email TEXT NOT NULL, last_sent_at INTEGER, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL);
    DELETE FROM speaker_tasks; DELETE FROM agenda_items; DELETE FROM evaluations; DELETE FROM submission_tracks; DELETE FROM tracks; DELETE FROM participations; DELETE FROM people; DELETE FROM submissions; DELETE FROM waves; DELETE FROM formats; DELETE FROM rooms; DELETE FROM buildings; DELETE FROM events; DELETE FROM api_tokens; DELETE FROM calendar_invites;
    INSERT INTO events VALUES ('${EVENT_ID}', 'Dashboard Fixture Conference', 'dashboard-fixture', 'America/New_York');
    ...
    INSERT INTO people VALUES ('person-1', 'Amara van der Meer', 'amara@example.com', 'Marquee Systems');
    INSERT INTO buildings VALUES ('building-1', '${EVENT_ID}', 'Sheraton', NULL, NULL, NULL);
    INSERT INTO rooms VALUES ('room-1', '${EVENT_ID}', 'building-1', 'Liberty 3');
    INSERT INTO submissions VALUES
      ('sub-submitted', 'SUB-1', '${EVENT_ID}', 'abstract', 'Submitted work', NULL, 'submitted', 'fmt-stage', NULL, 'public', ${now}, ${now}, 'submitted work'),
      ('sub-review', 'SUB-2', '${EVENT_ID}', 'abstract', 'Review work', NULL, 'in_review', 'fmt-stage', NULL, 'public', ${now}, ${now}, 'review work'),
      ('sub-unplaced', 'SUB-3', '${EVENT_ID}', 'abstract', 'Unplaced accepted work', NULL, 'accepted', 'fmt-stage', 'wave-1', 'public', ${now}, ${now}, 'unplaced work'),
      ('sub-waved', 'SUB-4', '${EVENT_ID}', 'abstract', 'Pending wave work', NULL, 'accepted', 'fmt-stage', 'wave-2', 'public', ${now}, ${now}, 'pending wave work'),
      ('sub-scheduled', 'SUB-5', '${EVENT_ID}', 'abstract', 'Private scheduled work', NULL, 'accepted', 'fmt-stage', 'wave-2', 'public', ${now}, ${now}, 'private scheduled work'),
      ('sub-published', 'SUB-6', '${EVENT_ID}', 'session', 'Published work', NULL, 'accepted', 'fmt-stage', 'wave-2', 'admin', ${now}, ${now}, 'published work'),
      ('sub-rejected', 'SUB-7', '${EVENT_ID}', 'abstract', 'Workshop rejected work', NULL, 'rejected', 'fmt-workshop', NULL, 'public', ${now}, ${now}, 'rejected work');
    ...
    INSERT INTO agenda_items VALUES
      ('agenda-private', '${EVENT_ID}', 'sub-scheduled', 'session', ${now}, 30, 'room-1', 0),
      ('agenda-public', '${EVENT_ID}', 'sub-published', 'session', ${now}, 30, 'room-1', 1);
