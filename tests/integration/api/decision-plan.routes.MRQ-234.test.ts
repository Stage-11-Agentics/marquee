...
describe.sequential("MRQ-234 decision plan routes", () => {
  ...
  test("AC-380 · MRQ-234 · bulk plan returns four rows, a real rendered recipient, feedback echo, demo truth, and strong fingerprint", async () => {
  ...
  test("AC-380, AC-382 · MRQ-234 · single-record plan uses the same contract and does not fabricate mail for waitlist", async () => {
  ...
  test("AC-381 · MRQ-234 · apply refuses a changed email with authored 409, then accepts a fresh positive plan", async () => {
  ...
  test("AC-381 · MRQ-234 · missing precondition is 400 and an all-skipped apply is a zero-effect 409", async () => {
  ...
  test("AC-382 · MRQ-234 · mixed apply preserves per-record post-send failures", async () => {
    const body = {
      selector: { ids: ["sub-mrq234-plan-good", "sub-mrq234-plan-invalid"] },
      action: "reject",
    };
  ...
  test("AC-380 · MRQ-234 · Notify plan reuses the four rows with queued-copy truth and a rendered real recipient", async () => {
    await env.DB.batch([
      env.DB.prepare(
