...
const EXPECTED_PLACEHOLDER_SITES = [
  ...
  },
  {
    ...
    owner: "drainMirrorOutbox",
    binding: "drained",
    expression: 'claimed.map(() => "?")',
    classification: "bounded by the queue consumer's 100-row claim limit, and only used to mark the claimed outbox rows drained",
  },
  {
    file: "src/jobs/mirror/consumer.ts",
    owner: "markDrained",
    binding: "placeholders",
    expression: 'rows.map(() => "?")',
    classification: "bounded by the queue consumer's 100-row claim limit; status bookkeeping only, never a user-sized write",
  },
  {
    file: "src/jobs/mirror/consumer.ts",
    owner: "markFailed",
    binding: "placeholders",
    expression: 'rows.map(() => "?")',
    classification: "bounded by the queue consumer's 100-row claim limit; failed-status bookkeeping only, never a user-sized write",
  },
  {
    file: "src/jobs/mirror/outbox.ts",
    owner: "clearMirrorOutbox",
    binding: "placeholders",
    expression: 'MIRRORED_TABLES.map(() => "?")',
    classification: "exactly the three-table MIRRORED_TABLES taxonomy; disabled cleanup cannot expand with user input",
  },
  {
    file: "src/jobs/calendar/batch.ts",
    owner: "readLedgerFloors",
    binding: "placeholders",
    expression: 'group.map(() => "?")',
    classification: "chunked at 80 UIDs per query (MAX_D1_IN_PLACEHOLDERS); the debt-item list it reads from is bounded only by how much of the event is currently stale, not by any request-time cap",
  },
  {
    file: "src/jobs/calendar/batch.ts",
    owner: "readExistingBatchOwners",
    binding: "personPlaceholders",
    expression: 'group.map(() => "?")',
    classification: "chunked at 80 person IDs per query, leaving headroom for the fixed event_id binding below D1's 100-binding cap; the speaker list it reads from is bounded only by how many speakers currently have calendar debt, not by any request-time cap",
  },
  {
    file: "src/jobs/calendar/batch.ts",
    owner: "readExistingBatchOwners",
    binding: "ownerPlaceholders",
    expression: 'group.map(() => "?")',
    classification: "chunked at 80 owner IDs per query; one person can accumulate more prior batch-owner rows than personIds.length, so this is chunked independently of the person-ID read above",
  },
  {
    file: "src/jobs/calendar/resolver.ts",
    owner: "resolveCalendarIcs",
    binding: "placeholders",
    expression: 'group.map(() => "?")',
    classification: "chunked at 80 owner IDs per query; one UID can accumulate an owner row per reschedule/resend over the event's life with no enforced ceiling, so the public ICS resolver chunks rather than assuming a small history",
  },
  {
    file: "src/jobs/mail/consumer.ts",
    owner: "calendarPartsFor",
    binding: "placeholders",
    expression: 'batchIds.map(() => "?")',
    classification: "bounded by the marquee-mail queue's max_batch_size: 10 (wrangler.jsonc); processMailOutbox's only production caller is the queue consumer, so batchIds can never approach D1's binding cap",
  },
  {
