...
const llmsFullPath = resolve(repositoryRoot, "src/agent-front-door/llms-full.txt");
...
test("CONTRACT · MRQ-257 final Worker build serves exact generated bytes and its build beacon", async () => {
  const { app } = await import(`${resolve(bundlePath)}?mrq-257-fresh=${Date.now()}`);
  ...
  const [servedLlms, servedFull, expectedLlms, expectedFull] = await Promise.all([
    request("/llms.txt").then((response) => response.text()),
    request("/llms-full.txt").then((response) => response.text()),
    readFile(llmsPath, "utf8"),
    readFile(llmsFullPath, "utf8"),
  ]);
  assert.equal(servedLlms, expectedLlms);
  assert.equal(servedFull, expectedFull);

  const health = await request("/health");
  assert.equal(health.status, 200);
  const healthBody = await health.json();
  const expectedBuild = (process.env.MARQUEE_BUILD_SHA ?? process.env.GITHUB_SHA
    ?? spawnSync("git", ["rev-parse", "--short=12", "HEAD"], { cwd: repositoryRoot, encoding: "utf8" }).stdout.trim())
    .slice(0, 12);
  assert.equal(healthBody.build, expectedBuild);
});
