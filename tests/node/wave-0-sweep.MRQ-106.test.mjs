...
test("CONTRACT · MRQ-106 · the route map is generated and gated, never hand-written", async () => {
  ...
  // The route claims that shipped false twice. `/comms` is
  // `/communications`; and `/agenda` is the public site, not the organizer's
  // builder. `/settings/webhooks` is now a real organizer route, generated from
  // the route table rather than hand-added to this document.
  //
  // `/site` was on this list for the same reason — the notes advertised it and
  // the build did not serve it. It is answered now (public-agenda.route.tsx
  // redirects it to /agenda), so the map listing it is the truth rather than
  // the old lie, and the guard that matters is the generated one below: this
  // file is written by check:routes from the route sources, so a path can only
  // appear here by being served.
  assert.match(map, /`\/site`/);
  assert.match(map, /`\/settings\/webhooks`/);
