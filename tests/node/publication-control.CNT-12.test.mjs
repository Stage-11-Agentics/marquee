...
test("CONTRACT · CNT-12 the record slot owns the publication control and reserves its geometry", async () => {
  ...
  assert.match(page, /is_published: boolean/);
  assert.match(page, /record\.is_published/);
  assert.match(page, /record-publication-status/);
  ...
  assert.match(css, /\.record-publication-status[^}]*min-width: 164px/);
