...
test("CONTRACT · same-path GET and POST policies do not bleed into each other", async () => {
  ...
  const post = await router.app.request("https://marquee.example/api/v1/org/tokens", { method: "POST" }, {} as never);
  expect(post.status).toBe(200);
  expect(await post.json()).toEqual({ method: "post" });
  expect(buckets).toEqual(["write"]);

  const get = await router.app.request("https://marquee.example/api/v1/org/tokens", {}, {} as never);
