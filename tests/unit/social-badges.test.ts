...
describe("the badges a speaker's profiles get", () => {
  test("CONTRACT · a profile link carries the platform in its accessible name, not just its mark", () => {
  ...
  test("CONTRACT · the canonical destination is used even when the stored link is the old host", () => {
  ...
  test("CONTRACT · every outbound link carries rel=me and opener hygiene", () => {
  ...
  test("CONTRACT · a link on no shipped platform is still shown, under its host", () => {
    expect(render(["https://example.com/talks"])).toContain("example.com");
  });

  test("CONTRACT · a speaker with no links renders nothing at all", () => {
    expect(render([])).toBe("");
  });
});
