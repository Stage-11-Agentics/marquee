...
test("AC-391 · a running program keeps a live compact setup row and routes its next step", async () => {
...
test("AC-391 · completed compact setup remains a reserved slot after dismissal", async () => {
  vi.stubGlobal("fetch", vi.fn(async (input: unknown) => {
    const url = requestUrl(input);
    if (url.includes("/forms")) return okResponse({ data: [{ status: "open" }] });
...
test("AC-392 · onboarding says acceptance is the prerequisite, not all-clear", async () => {
...
test("AC-392 · an accepted speaker with no open tasks gets the genuine all-clear", async () => {
  vi.stubGlobal("ResizeObserver", class { observe() {} disconnect() {} });
...
test("AC-393 · communications names the acceptance prerequisite before offering a recipient action", async () => {
  vi.stubGlobal("fetch", vi.fn(async (input: unknown) => {
    const url = requestUrl(input);
...
test("AC-393 · communications does not call an audience failure an empty prerequisite", async () => {
  vi.stubGlobal("fetch", vi.fn(async (input: unknown) => {
    const url = requestUrl(input);
...
test("AC-393 · agenda empty state opens the unfiltered submissions surface", async () => {
  const snapshot = {
...
test("AC-394 · the shared demo signal hides reset on a non-demo account and keeps the footer reachable", async () => {
