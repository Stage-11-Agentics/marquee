...
describe("MRQ-114 · CNT-01 · task due dates", () => {
  test("every fixture date survives the round trip through storage", () => {
  ...
  test("the stored instant is the end of the named day, not its start", () => {
    const stored = dueAtFromDateInput("2027-05-01") as number;
    expect(new Date(stored).toISOString()).toBe("2027-05-01T23:59:59.999Z");
  });

  test("the rendered date reads the same in every timezone", () => {
  ...
  test("malformed and impossible dates are refused rather than rolled forward", () => {
    for (const value of ["", "not-a-date", "2027-5-1", "2027-02-30", "2027-13-01", "2027-00-10"]) {
      expect(dueAtFromDateInput(value), value).toBeNull();
    }
  });

  test("an offset deadline counts from now; a fixed one ignores now entirely", () => {
