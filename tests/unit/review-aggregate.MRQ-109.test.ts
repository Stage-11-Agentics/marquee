...
describe("MRQ-109 · the results table's ordering and its honesty about the number", () => {
  test("CONTRACT · ABS-10: the score sort exists in both directions", () => {
  ...
  test("CONTRACT · ABS-10: unscored submissions sort last in BOTH directions", () => {
  ...
  test("CONTRACT · a sort without the flag keeps the plain clause", () => {
    expect(orderClause({ column: "s.updated_at", direction: "desc" })).toBe("s.updated_at DESC, id ASC");
  });

  test("CONTRACT · ABS-04: a score never appears without the reviewer count behind it", () => {
    expect(reviewCountLabel(0)).toBe("Not scored");
    expect(reviewCountLabel(1)).toBe("1 review");
    expect(reviewCountLabel(4)).toBe("4 reviews");
  });

  test("CONTRACT · a fallback score is labelled unweighted rather than silently relabelled", () => {
