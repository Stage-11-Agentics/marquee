...
describe("MRQ-234 pure decision planner", () => {
  test("always returns the four rows, including an empty muted shape", () => {
  ...
  test("classifies the signed truth table without dropping records", () => {
  ...
  test("disabled templates and demo suppression stay advisory on the sendable row", () => {
    const plan = planBulkDecision({
      action: "reject",
  ...
  test("waitlist and withdraw are no-mail actions, not address failures", () => {
  ...
  test("feedback is normalized in the pure contract", () => {
...
describe("MRQ-234 shared email validity", () => {
  ...
  test("the SQL predicate has the same named shape instead of the old LIKE twin", () => {
