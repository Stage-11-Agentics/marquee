...
async function survivingEventDemoPeopleBlockers(
  ...
  const checks: Array<[string, string, string]> = [
    ...
    ["speaker_helpers.speaker_person_id", "speaker_person_id", "SELECT r.id AS row_id, r.event_id, r.speaker_person_id AS person_id FROM speaker_helpers r JOIN selected_people p ON p.id = r.speaker_person_id JOIN surviving_events e ON e.id = r.event_id"],
    ["speaker_helpers.helper_person_id", "helper_person_id", "SELECT r.id AS row_id, r.event_id, r.helper_person_id AS person_id FROM speaker_helpers r JOIN selected_people p ON p.id = r.helper_person_id JOIN surviving_events e ON e.id = r.event_id"],
    ["speaker_helpers.added_by", "added_by", "SELECT r.id AS row_id, r.event_id, r.added_by AS person_id FROM speaker_helpers r JOIN selected_people p ON p.id = r.added_by JOIN surviving_events e ON e.id = r.event_id"],
...
export async function deleteEventCascade(
  ...
  statements.push(
    ...
    prepared(db, `DELETE FROM speaker_helpers WHERE event_id IN ${eventIdsSql}`, ...eventBindings),
...
function eventOwnedStatements(
  ...
  return [
    ...
    prepared(db, "DELETE FROM speaker_helpers WHERE event_id IN " + eventIdsSql, ...bindings),
