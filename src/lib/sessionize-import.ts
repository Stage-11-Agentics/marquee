...
async function importSession(
  ...
  if (!current) {
    await withSubmissionReferenceAllocation(db, event.id, now, (referenceCode) => [
      db.prepare(
      ...
      ).bind(id, event.id, referenceCode, next.formId, next.title, next.abstract, next.status, next.formatId, next.trackId, next.submitterId, now, now, externalRef, now, now),
...
async function restoreSnapshot(db: D1Database, snapshot: ImportSnapshot): Promise<void> {
  ...
  await db.batch([
    ...
    ...(snapshot.tracks ?? []).map((row) => db.prepare("INSERT INTO submission_tracks (id, submission_id, track_id, is_primary, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET submission_id = excluded.submission_id, track_id = excluded.track_id, is_primary = excluded.is_primary, updated_at = excluded.updated_at").bind(row.id, row.submission_id, row.track_id, row.is_primary, row.created_at, row.updated_at)),
