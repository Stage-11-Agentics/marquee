...
const DOCUMENT_CONFIG = {
  ...
  info: {
    ...
    description: [
      ...
      "",
      "Optimistic concurrency covers **agenda items** and decision-plan applies, where concurrent editing is real:",
      ...
      "`ETag` response header. Bulk and single submission decision applies require the decision",
      "plan's strong ETag in `If-Match` and refuse a stale preview with `409`. Several mutations",
      "still refuse a concurrent change on their own terms — agenda publication, participation",
      "responses, and task completion answer `409` when the record moved underneath the request;",
      "decision applies can also report per-record transition drift with `422` — so a `409` or",
      "`422` is worth handling on any write.",
    ].join("\n"),
  },
  servers: [
    {
