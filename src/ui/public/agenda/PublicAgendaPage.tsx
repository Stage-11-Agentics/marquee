...
export function PublicAgendaPage({ data }: { data: PublicAgendaData }): JSX.Element {
  ...
  return (
    <PublicShell
      event={data.event}
      title="Agenda"
      actions={<><a class="public-button" href={`/speakers?${eventQuery}`}>Speakers</a><a class="public-button" href={`/api/v1/public/agenda?${eventQuery}`}>Agenda data ↗</a><a class={`public-button ${data.sessions.length > 0 ? "primary" : ""}`.trim()} href={`/embed/config?${eventQuery}`}>Get embed code</a></>}
    >
