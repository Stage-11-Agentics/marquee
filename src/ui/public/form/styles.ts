...
.public-form { min-height: 100vh; background: var(--bg); color: var(--ink); }
...
.public-form-main { width: min(900px, calc(100% - 32px)); margin: 0 auto; padding: clamp(30px, 7vw, 76px) 0 84px; }
...
.public-form-card { background: var(--panel); border: 1px solid var(--line-strong); border-radius: var(--radius); margin-top: 26px; }
...
.public-field input:not([type=checkbox]):not([type=radio]), .public-field textarea, .public-field select { background: var(--surface); border: 1px solid var(--line-strong); border-radius: var(--radius); font-size: 14px; min-height: 42px; outline: none; padding: 10px 11px; width: 100%; }
...
.public-field-error { color: var(--alarm); font-size: 12px; line-height: 1.45; }
...
@media (max-width: 560px) {
  ...
  .public-form-main { width: calc(100% - 24px); padding-top: 30px; }
  .public-intro h1 { font-size: clamp(29px, 11vw, 42px); }
  .public-intro p { font-size: 13px; }
  .public-form-fields { padding: 19px 14px; }
  .public-form-footer { align-items: stretch; flex-direction: column; padding: 14px; }
