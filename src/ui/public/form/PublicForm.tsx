...
export function PublicForm({ initial }: PublicFormProps) {
  ...
  if (state.state === "submitted" && state.confirmation && !editingSubmitted) {
    return <div class="public-form"><PublicHeader state={state} /><main class="public-form-main"><section class="public-confirmation" aria-live="polite" data-submission-outcome={state.outcome ?? "pending"}><div class="public-brand-mark">✓</div><h2>{state.confirmation.title}</h2><p>{state.confirmation.message}</p><p>We will write to <strong>{state.confirmation.email}</strong>.</p>{state.submission_edit_reason && <p class="public-confirmation-edit-note" role="status">{state.submission_edit_reason}</p>}{state.resume_url && <p class="public-resume">Save this link to reopen this confirmation later; the same link is in your confirmation email. <a class="public-resume-link" href={resumeLinkPath(state.resume_url)}>{resumeLinkPath(state.resume_url)}</a></p>}{state.confirmation.portal_url && <p>This link is your sign-in. <a href={state.confirmation.portal_url}>Track your submission →</a></p>}</section></main><PublicFooter /></div>;
  }
