...
export function VenuesPage({ eventId }: { eventId?: string }): JSX.Element {
  ...
  return <div class="venues-page">
    <PageHeader title="Venues" copy="A conference can span more than one door. Keep the map, arrival instructions, and room capabilities in one place." actions={<button class="button primary" disabled={!model || saving} onClick={save}>{saving ? "Saving…" : "Save venues"}</button>} />
    ...
    {model && <>
      <section class="card venue-map-card"><div class="card-head"><div><h2>Site map</h2><span class="subtle">Pins use verified building coordinates · lines show walking time</span></div><span class="chip">{model.buildings.filter((building) => building.lat !== null && building.lng !== null).length} of {model.buildings.length} pinned</span></div><div class="card-body"><VenueMap buildings={model.buildings} /></div></section>
      <div class="venues-editor-grid">
        <section class="card"><div class="card-head"><div><h2>Buildings</h2><span class="subtle">Coordinates drive the map and Transit checks</span></div><button class="button small" onClick={addBuilding}>+ Add building</button></div><div class="card-body venue-list">{model.buildings.map((building) => <article class="building-editor" key={building.id}>
