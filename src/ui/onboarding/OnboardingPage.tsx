  ...
  return <div class="onboarding-page">
    ...
    {ready ? <>
      ...
      <section class="onboarding-board card" aria-label="Speaker onboarding chase board">
        ...
        {state.kind === "ready" && (rows.length > 0 || page > 1) ? <div class="onboarding-tablefoot"><span class="tabular">Showing {rows.length} of {matchingTotal} speakers</span><span class="onboarding-pager"><Button small disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>Previous</Button><span class="onboarding-pager-label tabular">Page {ready.page ?? page} of {totalPages}</span><Button small disabled={page >= totalPages} onClick={() => setPage((current) => current + 1)}>Next</Button></span></div> : null}
      </section>
