...
function SpeakerDrawer({ eventId, personId, onClose }: { eventId: string; personId: string; onClose: () => void }): JSX.Element {
  ...
  return <aside class="onboarding-drawer" role="dialog" aria-modal="true" aria-labelledby="speaker-drawer-title">
    ...
    {state.kind === "ready" ? <div class="onboarding-drawer-body">
      <section class="onboarding-profile"><div class="onboarding-avatar" aria-hidden="true">{initials(state.detail.person.name)}</div><div><strong>{state.detail.person.name}</strong><span>{state.detail.person.email}</span><span>{[state.detail.person.title, state.detail.person.company].filter(Boolean).join(" · ") || "—"}</span></div></section>
  ...
  return <div class="onboarding-page">
    ...
    {ready ? <>
      ...
      <section class="onboarding-board card" aria-label="Speaker onboarding chase board">
        ...
        {state.kind === "ready" && rows.length === 0 ? <EmptyState title={hasActiveFilters ? "No speakers match these filters" : "Nothing outstanding"} copy={hasActiveFilters ? "Adjust the filters to see speakers with outstanding conference tasks." : "Every accepted speaker is clear. Completed and cancelled work remains in the record, while this board keeps only tasks the conference still owes."} action={hasActiveFilters ? <Button variant="primary" onClick={clearFilters}>Clear filters</Button> : <Button variant="primary" onClick={() => navigate?.("/submissions?status=accepted")}>Open accepted speakers</Button>} /> : null}
        {state.kind === "ready" && rows.length > 0 ? <div class="onboarding-matrix-wrap"><table class="onboarding-matrix"><thead><tr><th class="onboarding-select-column"><input type="checkbox" aria-label="Select all visible speakers" checked={allVisibleSelected} onChange={toggleAll} /></th><th scope="col">Speaker</th><th scope="col" class="onboarding-track-column">Track</th>{ready.task_templates.map((task) => <th scope="col" key={task.id}><span title={task.description}>{task.name}</span></th>)}<th scope="col">Last contact</th><th class="onboarding-action-column" scope="col"><span class="sr-only">Actions</span></th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td class="onboarding-select-column"><input type="checkbox" aria-label={`Select ${row.person.name}`} checked={selected.has(row.id)} onChange={() => toggleRow(row.id)} /></td><th scope="row"><button class="onboarding-speaker-link" type="button" onClick={(event) => openDrawer({ kind: "speaker", personId: row.person.id }, event.currentTarget)}><span class="onboarding-row-avatar" aria-hidden="true">{initials(row.person.name)}</span><span><strong>{row.person.name}</strong><small>{row.person.company || row.person.email} · {row.wave?.name ?? "No wave"} · {row.sessions.length} Session{row.sessions.length === 1 ? "" : "s"}</small></span></button></th><td class="onboarding-track-column">{row.tracks.length > 0 ? row.tracks.map((track) => <span class="onboarding-track" key={track.id}><i style={{ backgroundColor: track.color }} aria-hidden="true" />{track.name}</span>) : <span class="onboarding-muted">No track</span>}</td>{ready.task_templates.map((task) => { const cell = row.cells[task.id] ?? { glyph: "—", state: "unassigned", title: task.name, due_at: null, owed: false }; return <td key={task.id} class={`onboarding-cell state-${cell.state}`}><span class="onboarding-glyph" aria-label={`${cell.title}: ${cell.state}`}>{cell.glyph}</span><small>{cell.state === "unassigned" || cell.state === "cancelled" ? "—" : cell.due_at === null ? "—" : formatDate(cell.due_at)}</small></td>; })}<td class="onboarding-last-contact">{formatDate(row.last_contact)}</td><td class="onboarding-action-column"><Button small onClick={(event) => selectForNudge(row, event)}>Nudge</Button></td></tr>)}</tbody></table></div> : null}
      </section>
