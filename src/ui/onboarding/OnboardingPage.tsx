...
interface PortalInviteResponse {
  ...
  /** Recipients the batch could not invite, each named with the reason. */
  skipped: Array<{ person_id: string; name: string; reason: string }>;
}
 ...
 *
 * The board is wider than the speaker roster on purpose — it also chases a
 * sponsor's contact, who works through the sponsor portal and holds no speaker
 * seat. Saying so on the row is the difference between a control the operator
 * knows not to use and one that looks live and refuses.
 */
const NO_PORTAL_SEAT = "No speaker seat at this conference, so the speaker portal cannot let them in.";
...
function SpeakerDrawer({ eventId, personId, onClose }: { eventId: string; personId: string; onClose: () => void }): JSX.Element {
  ...
  return <aside class="onboarding-drawer" role="dialog" aria-modal="true" aria-labelledby="speaker-drawer-title">
    ...
    {state.kind === "ready" ? <div class="onboarding-drawer-body">
      ...
      {/* Arrivals sit under Sessions because they are the same list read on the
          day: one line per session this person was actually seen for, and who
          said so. A speaker with no marks yet is not shown a section at all —
          before the show day there is nothing here but noise. */}
      {state.detail.arrivals.length > 0 ? <section class="onboarding-drawer-section"><h3>Marked in</h3><div class="onboarding-context-list">{state.detail.arrivals.map((arrival) => <div class="onboarding-context-row" key={arrival.agenda_item_id}><span class="onboarding-context-mark">●</span><div><strong>{arrival.session_title}</strong><span>{arrival.room_name} · {formatDateTime(arrival.starts_at)} · marked by {arrival.marked_by_name}</span></div></div>)}</div></section> : null}
...
export function OnboardingPage({ eventId, search = "", navigate }: { eventId: string; search?: string; navigate?: (target: string) => void }): JSX.Element {
  ...
  const invitableSelectedRows = selectedRows.filter((row) => row.portal_invitable);
  const uninvitableSelectedRows = selectedRows.filter((row) => !row.portal_invitable);
  ...
  const inviteSelected = async () => {
    // Only the rows that can actually receive one. The server still names
    // anyone it cannot invite, but an operator should not have to send a batch
    // to learn what the board already knows.
    const personIds = [...new Set(invitableSelectedRows.map((row) => row.person.id))];
    if (personIds.length === 0) return;
    setInviteState({ kind: "sending" });
    try {
      const result = await requestJson<PortalInviteResponse>(`/api/v1/events/${encodeURIComponent(eventId)}/speakers/invite`, "/api/v1/events/{eventId}/speakers/invite", {
