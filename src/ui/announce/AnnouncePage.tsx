...
export function AnnouncePage({ eventId }: { eventId: string }): JSX.Element {
  ...
  return <div class="announce-page">
    ...
    {(plan || planLoading || planError || planStale) ? <DecisionPlanPanel plan={plan} loading={planLoading} error={planError} stale={planStale} busy={applyBusy} feedback="" confirmPublished={false} publishedCount={null} onFeedbackChange={() => undefined} onConfirmPublishedChange={() => undefined} onConfirm={() => void applyPlan()} onClose={() => { setPlan(null); setSelection(null); }} onRefresh={() => { if (selection) void openPlan(selection); }} /> : null}
