...
function conditionSummary(condition: FormCondition | null): string {
    ...
    <div class="forms-builder">
      ...
      <section class="card forms-editor" aria-label="Form editor">
        ...
        <CardBody>{step === 2 ? <>
          ...
          <div class="forms-field-list">{[...form.fields].sort((left, right) => left.position - right.position).map((field, index) => <button key={field.id} class={`forms-field-row ${field.id === selectedFieldId ? "active" : ""}`} data-builder-field={field.key} onClick={() => setSelectedFieldId(field.id)}><span class="forms-drag-handle" aria-hidden="true">⋮⋮</span><span class="forms-field-order">{String(index + 1).padStart(2, "0")}</span><span class="forms-field-copy"><strong data-field-label={field.label}>{field.label}{field.required ? " *" : ""}</strong><small>{fieldTypeLabel(field.type)} · {field.required ? "Required" : "Optional"}{field.condition ? ` · When ${conditionSummary(field.condition)}` : ""}</small></span><span class="forms-field-actions"><span class="chip">{field.type}</span><span class="forms-arrow" aria-hidden="true">→</span></span></button>)}</div>
