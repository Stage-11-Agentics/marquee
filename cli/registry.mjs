...
export const COMMAND_REGISTRY = [
  ...
  {
    path: ["mirror", "map"],
    usage: "marquee mirror map --base-id <base-id> --set submissions=<table-id> --set speaker_tasks=<table-id> --set people=<table-id> --airtable-token <token> [--provision]",
    ...
    options: [
      SET_OPTION,
      { name: "--base-id <base-id>", description: "Airtable base ID for this mapping request." },
  ...
  },
  {
    ...
    skill: "setup",
    event: true,
    set: ["name", "slug", "kind", "closes_at", "per_submitter_limit", "submitter_limit_inherit", "min_speakers", "max_speakers", "welcome_md"],
    options: [SET_OPTION],
  },
  {
    path: ["forms", "list"],
    usage: "marquee forms list <event-id>",
    summary: "List the conference's forms, publication state, inheritance, and effective capacity.",
  ...
  },
  {
    ...
    skill: "configure",
    event: true,
    set: ["name", "tagline", "starts_on", "ends_on", "timezone", "venue", "logo_key", "accent", "submission_default_limit"],
    options: [SET_OPTION],
  },
  {
    path: ["event", "delete"],
  ...
  },
  {
    ...
    summary: "Plan, then accept every submission selected by a server-side filter.",
    operations: ["bulkDecideSubmissions"],
    skill: "triage",
    event: true,
    options: [
      { name: "--filter <key=value>", description: "Required; repeat or pass a JSON filter object." },
      { name: "--feedback <text>", description: "Optional feedback rendered into each decision message." },
      { name: "--confirm-published", description: "Allow the plan to include records already live on the public agenda." },
    ],
  },
  {
    path: ["submissions", "reject"],
    usage: "marquee submissions reject <event-id> --filter <key=value>",
    summary: "Plan, then reject every submission selected by a server-side filter.",
    operations: ["bulkDecideSubmissions"],
    skill: "triage",
    event: true,
    options: [
  ...
  {
    path: ["submissions", "plan"],
    usage: "marquee submissions plan <event-id> --action <accept|reject|waitlist|withdraw|notify> [--filter <key=value>]",
    summary: "Read a bounded decision or Notify plan with its fingerprint, ETag, and disposition rows.",
    operations: ["planBulkSubmissionDecision", "planDecidedSubmissionsNotification"],
    skill: "triage",
    event: true,
    options: [
      { name: "--action <value>", description: "accept, reject, waitlist, withdraw, or notify." },
      { name: "--filter <key=value>", description: "Required for decision actions; repeat or pass a JSON filter object." },
      { name: "--feedback <text>", description: "Optional feedback rendered into each decision message." },
      { name: "--confirm-published", description: "Allow the plan to include records already live on the public agenda." },
    ],
  },
  {
    path: ["submissions", "apply"],
    usage: "marquee submissions apply <event-id> --action <accept|reject|waitlist|withdraw|notify> [--filter <key=value>]",
    summary: "Apply a previously reviewed plan with its strong precondition, or drain Notify with its queue revision.",
    operations: ["bulkDecideSubmissions", "notifyDecidedSubmissions"],
    skill: "triage",
    event: true,
    options: [
      { name: "--action <value>", description: "accept, reject, waitlist, withdraw, or notify." },
      { name: "--filter <key=value>", description: "Required for decision actions; use the same selector as the plan." },
      { name: "--plan-fingerprint <hex>", description: "Required for decision actions; copy the plan's fingerprint." },
      { name: "--if-match <etag>", description: "Required for decision actions; copy the plan's strong ETag." },
      { name: "--queue-revision <n>", description: "Required for Notify; copy the plan or prior page's queue revision." },
      { name: "--cursor <value>", description: "Continue a Notify page from its returned cursor." },
      { name: "--feedback <text>", description: "Optional feedback matching the plan." },
      { name: "--confirm-published", description: "Use the published-record confirmation captured by the plan." },
    ],
  },
  {
    path: ["review", "queue"],
  ...
  {
    path: ["helpers", "list"],
    usage: "marquee helpers list <event-id> <speaker-person-id>",
    summary: "List active helpers for one speaker, using the name the speaker gave.",
    operations: ["listEventSpeakerHelpers"],
    skill: "chase",
    event: true,
    options: [],
  },
  {
    path: ["helpers", "add"],
    usage: "marquee helpers add <event-id> <speaker-person-id> --set name=<name> --set email=<email>",
    summary: "Invite a helper for one speaker through the shared portal door.",
    operations: ["addEventSpeakerHelper"],
    skill: "chase",
    event: true,
    set: ["name", "email"],
    options: [SET_OPTION],
  ...
  {
    path: ["helpers", "remove"],
    usage: "marquee helpers remove <event-id> <speaker-person-id> <helper-person-id>",
    summary: "Remove a helper and close their conference-scoped access.",
    operations: ["removeEventSpeakerHelper"],
    skill: "chase",
    event: true,
    options: [],
  },
  ...
  {
    path: ["day-of", "run-of-show"],
    usage: "marquee day-of run-of-show <event-id> --day <YYYY-MM-DD>",
    summary: "Read one day as it will be run: rooms, sessions, who has arrived, and the state of each deck.",
    operations: ["getRunOfShow"],
    skill: "chase",
    event: true,
    options: [
      { name: "--day <YYYY-MM-DD>", description: "A conference-local day. Defaults to the conference's own today." },
    ],
  },
  {
    path: ["day-of", "slides"],
    usage: "marquee day-of slides <event-id> --day <YYYY-MM-DD> --state <name>",
    summary: "List the day's sessions in run order with the state of each deck.",
    operations: ["getSlidesBoard"],
    skill: "chase",
    event: true,
    options: [
      { name: "--day <YYYY-MM-DD>", description: "A conference-local day. Defaults to the conference's own today." },
      { name: "--room <room-id>", description: "Restrict to one room." },
      { name: "--state <name>", description: "all, received, missing, or overdue." },
    ],
  },
  {
    path: ["day-of", "links"],
    usage: "marquee day-of links <event-id>",
    summary: "List the conference's day-of links, live and revoked, without exposing a token.",
    operations: ["listDayOfLinks"],
    skill: "chase",
    event: true,
    options: [],
  },
  {
    path: ["day-of", "link"],
    usage: "marquee day-of link <event-id> --name <text> --kind <kind>",
    summary: "Mint a named day-of link. The URL is returned once and is never readable again.",
    operations: ["createDayOfLink"],
    skill: "chase",
    event: true,
    options: [
      { name: "--name <text>", description: "Required. Who holds it: \"Sam, front door\"." },
      { name: "--kind <kind>", description: "checkin (may mark arrivals) or green_room (looks only). Minting a green_room link rotates the previous one." },
    ],
  },
  {
    path: ["day-of", "revoke"],
    usage: "marquee day-of revoke <event-id> <link-id>",
    summary: "Revoke one day-of link, killing every copy of its URL.",
    operations: ["revokeDayOfLink"],
    skill: "chase",
    event: true,
    options: [],
  },
  {
    path: ["checkin", "mark"],
    usage: "marquee checkin mark <event-id> <session-id> --person <person-id>",
    summary: "Mark one speaker arrived for one session.",
    operations: ["markSpeakerArrived"],
    skill: "chase",
    event: true,
    options: [
      { name: "--person <person-id>", description: "Required. The speaker on that session." },
    ],
  },
  {
    path: ["checkin", "clear"],
    usage: "marquee checkin clear <event-id> <session-id> --person <person-id>",
    summary: "Take back an arrival mark.",
    operations: ["clearSpeakerArrival"],
    skill: "chase",
    event: true,
    options: [
      { name: "--person <person-id>", description: "Required. The speaker whose mark is being removed." },
    ],
  },
  {
    path: ["remind"],
