...
1. **Read state:** `lattice list`, open PRs, `c11 tree --all`. Note what moved since last tick. **When filtering `lattice list`, include `planned` as well as `in planning`** — they are distinct statuses, and a filter that omits `planned` makes a working delegator look like it never claimed. Full live set: `backlog, in planning, planned, in progress, review, in validation, pr_open, blocked`.
