...
- **MRQ-55 — the ICS spike returned only HALF a verdict. Read `spikes/s2-ics-clients/VERDICT.md` carefully and do not mistake its "Expected client behavior" section for findings; that section says of itself, "These are expectations to test, not observed results."**
  - **Settled, build to it:** the wire format and Resend delivery. Stable `UID`, `SEQUENCE` 0→1→2 across request/update/cancel, `METHOD:REQUEST`/`METHOD:CANCEL`, `STATUS:CONFIRMED`/`CANCELLED`, `ORGANIZER`, `ATTENDEE;RSVP=TRUE;PARTSTAT=NEEDS-ACTION`, `TZID:America/New_York` with an embedded `VTIMEZONE`, CRLF line endings with folded lines, plain-text + HTML + one `text/calendar; method=<METHOD>` alternative. Also settled: **Resend replaces the SMTP Message-ID before indexing**, so never resolve a sent message by exact Message-ID — resolve on the recipient/subject/send-time tuple.
  - **UNRESOLVED — no client verdict exists for any client.** Gmail is "Pending inbox inspection"; Outlook and Apple Calendar have not been sent at all because the addresses are operator-only. Nobody has yet observed whether any client shows RSVP controls, updates in place without leaving a duplicate, or honours the cancel.
  - Therefore: **build the ICS shape to the RFC** (RFC 5545 and iTIP RFC 5546), not to assumed client behaviour. Where a choice depends on how a client renders, pick the RFC-correct option, and **name it in your PR body as an explicit assumption pending the S-2 oracle** — one line per assumption. Do not write a test that asserts client behaviour nobody has observed, and do not fake the oracle locally. If you find yourself needing the verdict to proceed, say so and keep building the parts that do not depend on it.
