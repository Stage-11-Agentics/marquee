...
## Addendum — re-reviewed at branch HEAD `3ad29a0`

Three commits land after `494e219`, all surfaced by the local gate rather than by inspection, and all re-read:

- `465ef2c` — **CLI registry parity.** `check:api` failed on `cli-registry-parity`: a new versioned route must exist in `cli/api-registry.json`. Regenerated from the served OpenAPI document, and rather than register a route the CLI cannot call, added a real `marquee files list <event-id> --state <name>` verb over the same endpoint (`cli/registry.mjs`, `cli/marquee.mjs`, `SKILL.md` regenerated). That is the project's agent-native rule applied rather than dodged.
- `465ef2c` — **D1 placeholder inventory.** `tests/node/bulk-paths.AC-66-69.test.mjs` keeps a classified inventory of every dynamic `?`-expansion. The two chunked reads in `versions.ts` were `UNCLASSIFIED`; both are now registered with their bound: explicit 80-owner chunks (plus one `owner_type` binding on the attachments read) stay below D1's cap. The inventory was the right place to answer the question §Findings-1 raised about scale.
- `3ad29a0` — **`trace:ac` title contract.** Test titles must begin `AC-N ·` or `CONTRACT ·`. These tickets mint no AC IDs and minting is forbidden, so every title now reads `CONTRACT · MRQ-115[/CNT-nn] — …`, keeping the rubric ID it defends in the sentence.

No production behaviour changed after `494e219`; the diff since is registry data, one CLI verb, one test-inventory entry pair, and test titles. **Verdict stands: PASS at `3ad29a0`.**

**Local gate at `3ad29a0`: `{"command":"pr-gate","ticket":"MRQ-115","status":"pass","elapsedMs":92954,"budgetMs":120000}`** (1-minute load average 49 at start; the suite alone had failed its own 45 s objective at 77.7 s on an earlier run taken at load 110, which is the documented load caveat, not a defect — it passes inside the gate budget once the box is merely busy rather than saturated).
