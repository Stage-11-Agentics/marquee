...
Audit branch HEAD: 5bbd3eda4dd4ad9869424a61a27f676d54a7590d
...
5bbd3eda4dd4ad9869424a61a27f676d54a7590d; fullHistory true; 100 findings.
The report contains 83 denied-history-path entries (the checker repeats paths
from repeated objects), 11 denied-history-content labels, README/extension
findings, missing-license, and gitleaks-unavailable.
...
scripts/checks/repo-policy.mjs:1-29 it checks five denied path patterns and
...
Independent pattern scans found no credential-shaped literal outside
documented placeholders. The history scan did find
src/jobs/mail/consumer.ts:100, const apiKey = env.RESEND_API_KEY, but that is
an environment variable read with no secret value. This is useful negative
...
1. Exclude .lattice/** in its entirety. The tip has 770 .lattice path names,
   and the complete commit/path walk also has 770 unique .lattice names.
...
Commit 2eb34eb adds a fast tests/node guard for the recurring internal
...
- Branch rebased to forgejo/master before the plan and npm ci completed.
- 223 reachable commits and 3919 rev-list objects at the final audit HEAD.
