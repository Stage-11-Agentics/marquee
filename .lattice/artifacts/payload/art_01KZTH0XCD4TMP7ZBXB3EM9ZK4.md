...
## Addendum — the visual gap is now closed (c11 embedded browser, one surface, closed after)

The earlier failure had a cause worth recording, and it was **not** this ticket: `src/lib/cookies.ts:10,17` sets the session cookie `Secure`, and its own comment says "Safari and WKWebView drop a Secure cookie on an http:// origin". Every authenticated screen fails identically in a c11 browser against `http://localhost` — demonstrated on the pre-existing `/submissions`, which showed the same "Your session has expired". The repo already ships the opt-out (`INSECURE_LOCAL_COOKIES=1`, untracked `.dev.vars`, since removed).

With that set, driven from the landing page as an organizer would:

**Discoverability (the turn-budget surface).** The sidebar renders `▤Files`; one click lands on `/files`. The label is the noun, spelled exactly.

**The library, rendered:**

```
Files
Received 1 of 153 requested deliverables. Every upload keeps its history; …
REQUESTED 153 · RECEIVED 1 · MISSING 152 · OVERDUE 0
All deliverables 153 | Uploaded 1 | Missing 152 | Overdue 0
153 shown · 0 selected

FILE            SPEAKER          SESSION                             STATUS    UPLOADED       VERSIONS    SIZE
slides.pdf      Aarush Selvan    Going deep on Gemini Deep Research   UPLOADED  Aug 12, 2026   2 versions  69 B
—               Ahmad Awais      — / no session attached              AWAITING  due Aug 31…    —           —
—  Presentation Upload · marked complete, no file on record
                Alex Lieberman   — / no session attached              AWAITING  due Aug 31…    —           —
```

The uploaded row sorts to the top; empty cells hold an em-dash rather than collapsing; and the "marked complete, no file on record" line appears on exactly the seeded rows that claim done with nothing attached.

**Version history expanded (CNT-04, organizer half):**

```
VERSION HISTORY · AARUSH SELVAN · PRESENTATION UPLOAD
slides.pdf   V2 OF 2   uploaded Aug 12, 2026 at 3:44 AM   69 B   Download   Copy link
v2  slides.pdf  CURRENT · V2 OF 2   Aug 12, 2026 at 3:44 AM  69 B  Download …
```

**The portal, collapsed row (CNT-02, the rescue):**

```
Presentation Upload
file · due Aug 31, 2026
slides.pdf   V2 OF 2   uploaded Aug 12, 2026 at 3:44 AM
```

No expansion, no click: the speaker sees the filename and the version the conference is holding.

**One defect found by looking.** The header read "1 of 153 requested deliverables **are** in". Fixed to "Received 1 of 153 requested deliverables" — agreement-safe at every count. That is the browser pass earning its keep; no test would have caught it.

Housekeeping: exactly one browser surface open at a time, closed when done, dev server stopped, `.dev.vars` deleted. `c11 tree --all` reports 0 browser surfaces.
