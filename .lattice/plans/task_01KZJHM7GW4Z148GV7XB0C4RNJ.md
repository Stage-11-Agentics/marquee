...
## Runtime Schema Delta — SPEC Amendment 12 (RATIFIED)

Received from the Orchestrator during MRQ-2 implementation and before review/PR, then ratified in SPEC Amendment 12 at commit `f80d383`. This block overrides the original `attachments` checklist where they differ.

1. **Pending checksum lifecycle — RATIFIED.** `attachments.sha256` is nullable because no verified digest exists at presign time. Add nullable `r2_etag`; a `ready` row requires the provider-observed ETag. SHA-256 is populated only when R2 supplies a provider-verified checksum, never from a client declaration and never by overloading an MD5-shaped ETag.
2. **Draft/submission supporting files — RATIFIED.** Add `draft_file` and `submission_file` to the polymorphic `attachments.owner_type` relation. A durable draft capability may own a file before any submission row exists; submit later re-parents the ready attachment to `submission_file`/the created submission. Keep the generic `(owner_type,owner_id)` index and add partial owner lookup indexes for each new relation. No 47th table is required.
3. **Contract status.** SPEC Amendment 12 makes these requirements authoritative. No deviation remains, no AC was minted, and the next AC remains AC-254. Cite `SPEC Amendment 12` in MRQ-2's completion comment and stacked PR.
