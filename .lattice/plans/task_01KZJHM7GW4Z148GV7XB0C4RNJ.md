...
## Runtime Schema Delta — MRQ-14 Upload Review (AUTHORITATIVE)

Received from the Orchestrator during MRQ-2 implementation and before review/PR. This block overrides the original `attachments` checklist where they differ.

1. **Pending checksum lifecycle — ACCEPTED, DEVIATE-WITH-FLAG.** `attachments.sha256` is nullable because no verified digest exists at presign time. Add nullable `r2_etag`; a `ready` row requires the provider-observed ETag. SHA-256 is populated only when R2 supplies a provider-verified checksum, never from a client declaration and never by overloading an MD5-shaped ETag.
2. **Draft/submission supporting files — ACCEPTED, DEVIATE-WITH-FLAG.** Add `draft_file` and `submission_file` to the polymorphic `attachments.owner_type` relation. A durable draft capability may own a file before any submission row exists; submit later re-parents the ready attachment to `submission_file`/the created submission. Keep the generic `(owner_type,owner_id)` index and add partial owner lookup indexes for each new relation. No 47th table is required.
3. **Contract tension.** SPEC §3 originally enumerates only `person_headshot|task_upload|event_logo|import_file` and declares `sha256` without pending-state nullability or `r2_etag`. The implementation follows the later MRQ-14 upload-lifecycle finding because otherwise the first pending insert and public draft-file path are impossible. Flag this delta in MRQ-2's completion comment and stacked PR; do not mint an AC.
