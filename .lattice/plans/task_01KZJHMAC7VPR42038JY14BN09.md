...
## Implementation Phase Note (AUTHORITATIVE)

The Orchestrator subsequently ruled that MRQ-32 must proceed on independent work while MRQ-24 is still in flight. The independent slice owns the seven-template defaults and event-scoped enable/edit state, the `MAIL_SCHEDULE_CRON`/pre-close candidate read path, the MRQ-8-filtered audience read path, the per-recipient audience row model, and AC-tagged proofs including the UNIQUE outbox idempotency/delivery proof. It does not add a templated-send call site or a merge-field renderer.

MRQ-24's published seam was observed on `forgejo/mrq-24-chase @ b11982c`: `src/jobs/mail/render.ts`, `src/jobs/mail/merge-data.ts`, and the additive `recipientsFor(db, eventId, selector)` contract in `src/routes/comms.routes.ts`, including explicit-empty-selector no-op semantics. At the rebase boundary, retain those implementations and extend the single comms module/send path; do not fork or rewrite them.
