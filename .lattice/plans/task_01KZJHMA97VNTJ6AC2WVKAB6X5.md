...
- Add an integration test under `tests/integration/api/sessionize-import.AC-110-113.test.ts` using the real fixture files and a program-lead session. It will prove mapping/preview is write-free; AC-110 profile/status/relationship landing; AC-111 matched and unattributed score/comment rows; AC-112 by importing the exact same files twice and asserting all imported table row counts are identical with no duplicate external refs/emails; and AC-113 by undoing the batches and asserting imported rows are gone while a seeded unrelated submission/person/evaluation remains byte/count identical.
