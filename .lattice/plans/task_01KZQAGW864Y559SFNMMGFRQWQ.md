...
- Validation correction: the shipped seed pair starts concurrently in different buildings, so Transit must be additive to the person warning rather than an `else` branch. The shared `getConflicts` path now preserves room/person conflicts and also projects the helper's Transit result for that pair.
- Live observation on the local seeded Worker: dashboard showed `Conflicts 7`; agenda showed `⚠ 7 conflicts`; both affected Sheraton/Marriott tiles showed `⚠ Transit`; the drawer showed the exact helper message `Transit — 9 min walk to New York Marriott Marquis, plus 3 min building access. Needs 12 min; has 0.`; the building band showed Sheraton New York Times Square, New York Marriott Marquis, and Online. The legitimate `Hotel and Travel Reservations` task remained visible on the dashboard.
