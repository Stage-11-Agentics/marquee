...
- Reserve migration `0031` exactly for the additive
  `forms.submitter_limit_inherit INTEGER NOT NULL DEFAULT 0` column. `0029` is
  MRQ-242 and `0030` is MRQ-241. Verify this allocation before implementation and
  again after the required rebase; stop and report if a collision appears.
...
- At-limit state and refusal responses name the effective number and tell the
  submitter to use the saved resume link to continue a draft.
...
2. Add `migrations/0031_submission_capacity.sql`, its schema-delta receipt, the
   migration test harness import, the `FormRow` type, and the copy-manifest
   declaration. The migration is additive and defaults existing forms to
   explicit behavior; copied forms carry the flag with the form.
...
5. Load the event setting once at the public-form boundary and carry the
   ...
   at-limit sentence so its number and saved-resume next step cannot drift.
