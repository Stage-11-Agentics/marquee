...
- Cycle 1 plan head was reviewed by artifact `art_01M051M9NCBV1RPNE59M73DC77`: `77fa3a12`; Cycle 2 reviewed this plan-only head with artifact `art_01M053QAGQ67G7YP6WDES8Z6NS`: `0fe4e7c5e9d9b5b786e49aa6e11f6bbdbd421ef0`. The verified base is `github/main@52bb485f105e0392fe475332b87cbb48dbcee832`; this Cycle 2 resolver amendment is plan-only, will be the next branch head, and remains held in `in_planning` for Cycle 3 review.
...
Cycle 1 review found three load-bearing design gaps and one prototype mismatch. Cycle 2 identified one remaining resolver gap: source-priority lookup could hide a newer singular REQUEST or CANCEL for the same UID. This amendment closes that gap as a binding decision rather than leaving implementation options open. No source code is being changed in this cycle.
...
## Binding amendment decisions (Cycle 1 and Cycle 2)
...
- Recheck the migration directory only at implementation start and after rebase. Then add the next free migration with an `outbox_calendar_parts` child table for batch messages, ordered by `part_index`, carrying `outbox_id`, `submission_id`, `ics_uid`, the per-part `sequence`, filename, ICS body, content type, and created/updated timestamps. The child mapping is durable and is the explicit outbox-to-N-submissions audit map. Existing per-session messages remain on legacy singular `outbox.ics_uid`/`outbox.ics_body` for compatibility; a batch row uses the child parts as its canonical attachments.
- Update the schema mirror/types and any schema-delta checks for the child table; add migration coverage. Update event deletion and demo reset cleanup so child parts cannot outlive their batch outbox row. `GET /i/:uid.ics` must collect every candidate for the UID from both `outbox_calendar_parts` and legacy singular `outbox`, joining child parts to their owning outbox. Order revisions by the owning outbox revision/admission time (`outbox.created_at`) descending, then owning outbox `id` descending as the deterministic tie-breaker. Never source-prioritize child storage or use RFC `SEQUENCE` as the resolver sort: sequence remains lifecycle data, with child rows carrying stamped `sequence` and legacy rows parsing `SEQUENCE` from stored ICS. For one owning outbox that exposes both forms, use its recorded admission grain: an intended standalone per-session REQUEST/CANCEL selects the singular body, while an intended batch outbox selects the matching child attachment. Singular material wins only for the intended standalone grain; otherwise the child part is the batch attachment. A newer singular REQUEST or CANCEL for the same UID must therefore win over an older child part, while an older child can never hide a newer standalone revision. Batch text/HTML renders one stable `/i/:uid.ics` link per covered session.
...
- Add the migration-backed `outbox_calendar_parts` rows in stable `part_index` order. Each row carries `outbox_id`, `submission_id`, `ics_uid`, its stamped `sequence`, filename, ICS body, content type, and timestamps. Existing per-session outbox rows keep singular `ics_uid`/`ics_body`; batch outbox rows use the child parts. Render one stable `/i/:uid.ics` link per covered session, and make the resolver a cross-store newest-owner-revision read: union child and legacy candidates, join the owning outbox revision/admission time and ID, order by owner time then owner ID descending, and apply the recorded standalone-vs-batch admission grain when one owner exposes both forms. Preserve each candidate’s UID, method, and sequence; sequence is not the recency sort.
...
- resolver-focused proof for one UID: batch REQUEST followed by a newer standalone per-session REQUEST serves the newer singular body; batch REQUEST followed by a newer CANCEL serves the newer CANCEL body; equal owner timestamps select the higher owning outbox ID; same-owner fixtures prove standalone grain selects singular material and batch grain selects the child attachment, with each returned UID/METHOD/SEQUENCE preserved;
...
- `GET /i/:uid.ics` selects the newest owning outbox revision across child and legacy storage by owner revision time then owner ID, applies the intended standalone-vs-batch grain when one owner exposes both forms, and has focused batch→resend, batch→cancel, and equal-time tie proof.
...
1. This Cycle 2 amendment is plan-only: verify the two authoritative/branch plan copies are byte-identical, stage only the plan path, commit, push, and verify the exact amended branch head against `github/mrq-233-calendar-strip`. Do not implement source code.
2. Hold MRQ-233 in `in_planning` after the amendment. Do not transition back to `planned` or auto-fire Cycle 3. Report the exact amended head/hash and ask Adoption Orchestrator at c11 `workspace:10` / `surface:513`, mailbox `adoption-orchestrator`, to launch the fresh Cycle 3 reviewer separately.
3. When the orchestrator launches the released-slot Cycle 3 review, record its artifact against this exact head. Do not transition to implementation until that review passes and the orchestrator explicitly hands the ticket onward.
4. Only after Cycle 3 PASS, transition to `in_progress`; recheck upstream/base and the next migration number at implementation start and after rebase, then implement in small commits and push each durable unit. The post-CONSOLIDATION RESUME step, not this cycle, owns the MRQ-233 SPEC §3.8/§5.11 fold and stable US/AC allocation from the then-current mint (the current observed EVALUATION band is AC-337, but no ID is minted here).
