...
- `scripts/checks/check-seed.mjs` remains the single command wrapper and keeps the four MRQ-62 venue assertions. `scripts/checks/seed.ts` supplies the shared local Wrangler bootstrap/API client and the public-API shape, scale, B-3, and ugliness assertions; there is no second seed command.
- `scripts/checks/speed.ts` uses the same fresh real seed and local `wrangler dev` runtime. Playwright measures server-rendered/public and authenticated admin surfaces where the local build exposes them; API probes cover the mutation and queue mechanics. Every report records `local-wrangler-dev` and `deployed: false`; values are not presented as Cloudflare deployment evidence.
- `speed-report.json` keeps the existing 14-budget AC/objective classifier, adds per-probe method/sample data, and adds separate harness budgets for `check:seed` (30s), `check:speed` (4m), and the full `pr-gate` wall clock (30s). The latter is a harness budget, not a reclassified acceptance criterion.
- The unavailable/deployment-only public CFP and browser Long Tasks evidence are represented explicitly in the local report and named as MRQ-57 follow-ups; AC source labels remain unchanged and no AC budget is converted to an objective.
