...
Implement a real, event-scoped comment thread for each file deliverable slot. The durable owner is the slot (`owner_type` + `owner_id`, concretely the speaker task), never the current attachment. `attachment_id` is nullable metadata identifying the version the author was looking at. A v1 comment must remain visible after the speaker uploads v2, and an organizer reply must land in the same thread.
