...
on-switch. The screen shows the Airtable base link, Marquee and Airtable row
counts as of the last sync, queued versus stuck changes, last sync time, and
webhook expiry. The token is stored encrypted and is never shown again.
...
node cli/marquee.mjs mirror map --set submissions="$AIRTABLE_SUBMISSIONS_TABLE_ID" --set speaker_tasks="$AIRTABLE_TASKS_TABLE_ID" --set people="$AIRTABLE_PEOPLE_TABLE_ID" --airtable-token "$AIRTABLE_TOKEN" --url "$MARQUEE_URL" --token "$MARQUEE_TOKEN" --json
