...
| `/settings/airtable` | Airtable | organizer | — | client push |
...
| Path | Methods |
| --- | --- |
| `/__validation/session-cookie` | GET |
| `/health` | GET |
| `/mcp` | GET, POST |
| `/mirror/webhook` | POST |
| `/SKILL.md` | GET |
...
ROUTES (real paths, not hash routes):
  ...
  API & CLI: /api/docs
  Machine endpoints: /__validation/session-cookie /health /mcp /mirror/webhook /SKILL.md
```
